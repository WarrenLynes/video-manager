...
import { AuthFacade, VideosFacade } from '@video-manager/core-state';
...
export class VideosComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.popular$ = this.http.get('https://www.googleapis.com/youtube/v3/videos?part=snippet%2CcontentDetails%2Cstatistics&chart=mostPopular&regionCode=US')
      .pipe(map((x: any) => x.items)).subscribe();
  }
  ...
  saveVideo(video: any) {
    if(video.id) {
      ...
      this.http.post('/youtube/v3/videos?part=fileDetails%2Csnippet', formData).subscribe();
